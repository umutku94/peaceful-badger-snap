...
const getCaretCoordinates = (element: HTMLTextAreaElement, position: number, fontSize: number) => {
  ...
  const props = [
    ...
    'fontStyle', 'fontVariant', 'fontWeight', 'fontStretch', 'fontFamily',
  ...
  markerSpan.style.fontSize = `${fontSize}px`; // Apply font size to marker as well
...
const VentingTextBox = () => {
  ...
  const { animationSpeed, fallDistance, soundVolume, fontSize } = useSettings();
    ...
    if (newText.length > oldText.length) {
      ...
      if (textareaRef.current) {
        ...
        addedChars.split('').forEach((char, index) => {
          const { x, y } = getCaretCoordinates(textareaElement, cursorPosition - addedChars.length + index, fontSize);
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto mt-8 overflow-hidden">
      <textarea
        ref={textareaRef}
        className={cn(
          "w-full h-64 p-4 border border-gray-300 rounded-md resize-none focus:outline-none focus:ring-2 focus:ring-blue-500",
      ...
      {fallingLetters.map((letter) => (
        <span
          key={letter.id}
          className="absolute animate-fall-fade"
          style={{
            left: `${letter.x}px`,
            top: `${letter.y}px`,
            pointerEvents: 'none',
            ...
            fontSize: `${fontSize}px`, // Apply dynamic font size here
